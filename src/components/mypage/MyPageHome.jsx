  ...
  return (
    <div className="container max-w-[1280px] mx-auto px-4 sm:px-6 lg:px-8">
      <div className="border-t border-gray-300">
        <div className="text-gray-800 text-sm md:text-base font-medium flex items-center justify-start mb-4 mt-4">
          주문내역
        </div>
        <div className="flex items-center justify-center gap-4 md:gap-12 flex-nowrap mt-8 mb-10 overflow-x-auto px-2">
          {['입금전', '배송준비중', '배송중', '배송완료'].map((status, index, array) => (
            <React.Fragment key={status}>
              <div className="min-w-[80px] md:w-[100px] h-[80px] md:h-[100px] flex flex-col items-center justify-center bg-[#CBD5E1] text-center rounded-full shadow-md flex-shrink-0">
        ...
        <div className="flex justify-start md:justify-center gap-3 md:gap-6 mt-10 mb-12 flex-nowrap overflow-x-auto px-2">
          {['취소 0', '교환 0', '반품 0'].map((action) => (
            <Buttons
              ...
              size="large"
              state="default"
              className="min-w-[140px] md:w-[250px] h-[48px] md:h-[62px] border border-[#375785] bg-[rgba(226,232,240,0.2)] text-[#375785] rounded-full px-4 py-2 text-sm md:text-base flex-shrink-0"
            >
              {action}
            </Buttons>
          ))}
        ...
        <div className="flex justify-start md:justify-center gap-3 md:gap-6 flex-nowrap overflow-x-auto px-2">
          {['관심상품', '최근 본 상품', '나의 쿠폰'].map((label) => (
            <Buttons
              ...
              size="large"
              state="default"
              className="min-w-[140px] md:w-[250px] h-[48px] md:h-[62px] border border-[#375785] bg-[#375785] text-white rounded-full px-4 py-2 text-sm md:text-base flex-shrink-0"
            >
              {label}
            </Buttons>
          ))}
      ...
      <div className="text-center mt-4 mb-10 text-gray-600 text-sm md:text-base cursor-pointer hover:underline">
        로그아웃
      </div>
    </div>
  );
