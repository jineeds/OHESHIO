...
import { fetchBotResponse } from '../../components/main/ChatBotLogic'; // 위에서 만든 로직을 별도 파일로 분리했다고 가정
...
const LiveChat = ({ isOpen, onClose }) => {
    ...
    const [showSuggestions, setShowSuggestions] = useState(true); // 자주하는 질문 섹션 활성화
    ...
    useEffect(() => {
        const checkScreenSize = () => {
            setIsMobile(window.innerWidth < 1280); // 1280px 미만을 모바일/태블릿으로 간주
        };
    ...
    // 컴포넌트가 닫혀있으면 채팅 버튼만 표시
    if (!isOpen) return null;

    // 모바일 & 태블릿 화면에서 접힌 상태
    if (isMobile && isCollapsed) {
        return <div className='fixed bottom-16 right-4 z-40'></div>;
    }

    // 모바일 & 태블릿 레이아웃
    if (isMobile) {
        return (
            <div className='fixed right-0 bottom-20 z-40' ref={chatContainerRef}>
                <div
                    className='bg-white rounded-lg shadow-xl flex max-w-[340px] md:w-full flex-col overflow-hidden border border-gray-200'
                ...
                >
                    ...
                    <div className='flex items-center justify-between p-3 border-b'>
                        ...
                        <div className='flex items-center space-x-2'>
                            <button onClick={toggleChat} className='text-gray-500 p-1'>
                                <svg
                    ...
                    <div className='flex items-center p-2 border-b'>
                        <div className='flex items-center'>
                            <div className='w-5 h-5 rounded-md bg-primary-500 flex items-center justify-center text-white mr-2'>
                                <svg
                                    xmlns='http://www.w3.org/2000/svg'
                                    className='h-3 w-3'
                                    fill='none'
                            ...
                            <div>
                                <div className='text-xs font-medium font-korean'>오헤시오에 문의하기</div>
                    ...
                    {/* 메시지 영역 */}
                    <div className='flex-1 overflow-y-auto p-3 space-y-3' style={{ height: '285px' }}>
                        {chatMessages.map((msg) =>
                            msg.sender === 'agent' ? (
                                // 에이전트 메시지
                                <div key={msg.id} className='flex items-start'>
                                    <div className='w-2 h-2 rounded-full bg-primary-500 mr-2 mt-2'></div>
                                    <div>
                                        <div className='text-xs text-gray-500 mb-1'>Oheshio CS {msg.time}</div>
                                        <div className='bg-gray-100 p-2 rounded-lg inline-block max-w-xs'>
                                ...
                                <div key={msg.id} className='flex flex-col items-end'>
                                    ...
                                    <div className='bg-indigo-600 p-2 rounded-lg inline-block max-w-xs'>
                        ...
                        {isLoading && (
                            <div className='flex items-start'>
                                <div className='w-2 h-2 rounded-full bg-indigo-800 mr-2 mt-2'></div>
                                <div>
                            ...
                            <button
                                ...
                                className={`ml-2 rounded-full p-1 ${
                                    !message.trim() || isLoading
                                        ? 'text-gray-400'
                                        : 'text-primary-500 hover:bg-indigo-100'
                                }`}
                            >
                                <svg
                                    xmlns='http://www.w3.org/2000/svg'
